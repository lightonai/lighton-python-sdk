...
- **One `_request`** does auth header, error mapping (→ raises), and JSON parse. All calls route through it. A 2xx body that isn't JSON → `MalformedResponseError`, **unless `raw=True`**, which returns the 2xx body as `bytes` instead of parsing it. Two endpoints serve files rather than JSON (`GET /files/<id>/download` and `GET /files/<id>/thumbnail`) and this is the one decision on the list that got *edited* rather than extended. A flag on the existing chokepoint, not a `_download` sibling: auth, error mapping, the 429 cooldown and the rate gate are exactly what a sibling would have had to duplicate, and errors stay JSON even on a binary endpoint, so they must keep flowing through the same mapping (a test pins each: bytes back, a 404 still raising, a 429 still retrying). `raw` is keyword-only so it can't collide with an httpx kwarg, and an empty raw body is `b""`, not the `None` the JSON path returns. `_ActiveRecord._api` widened its `**kwargs` from `object` to `Any` to forward it.
...
- **`pending_reprocess` is a correctness requirement, not a nicety.** The API accepts a
  ...
  **previous** run until the work actually starts, along with the file-derived fields
  ...
  policy above.
- **`File.pages()`** reads back the text parsed at ingestion, `GET /files/<id>` with
  **`include_content=true`**, returning the **generated `Page`** from `types/api`,
  the very same model `parse` returns (the schema says so explicitly: "Defined once
  and reused everywhere, never redefine this shape per app"), so callers can switch
  between live parsing and an ingested file without reshaping. Deliberately a
  **method, not a field**, mirroring `facets()`: the flag is detail-only (the list
  endpoint has no `include_content`), and the text can be large, so it must not ride
  along on every `refresh()`. The response also carries a flat `content` string, but
  it is **deprecated** in the schema ("use `pages[]` instead"), so the SDK models
  neither it nor a `pages` field.
- **Binary content on `File`:** `download(purpose=DownloadPurpose.original)` and
  `download_thumbnail()`, both `raw=True` reads returning `bytes`. `download`'s
  `purpose` picks the stored version (`original`/`rendered_pdf`/`transcript`) and the
  server falls back to `original` when a rendition is missing, so it can't 404 for
  that reason. The **`thumbnail` field** (`Thumbnail` in `types/file.py`:
  `status`/`url`) exists so callers can check `ThumbnailStatus.READY` before fetching,
  because generation is asynchronous **and independent of ingestion**: verified live
  that an `embedded` file still reported `MISSING` and the fetch 404s. Note the
  method is `download_thumbnail()`, not `thumbnail()`, which the field name occupies.
